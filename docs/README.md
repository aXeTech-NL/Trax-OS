# Trax OS documentation

All product and platform documentation is organised under this directory. Repository governance, legal and security entry points remain at the repository root so GitHub and downstream distributions can discover them reliably.

## Platform

### Architecture

- [Architecture index](architecture/README.md)
- [Architecture decision log](architecture/decisions/README.md)
- [Canonical architecture glossary](architecture/GLOSSARY.md)
- [Agentic Core Architecture](architecture/AGENTIC_CORE.md)
- [Agency, Group Travel and Access Model](architecture/AGENCY_ACCESS_MODEL.md)
- [Atlas Model Provider Architecture](architecture/ATLAS_PROVIDER_MODEL.md)
- [V1 Agent-assisted Web Research](architecture/AGENT_RESEARCH_V1.md)
- [Implementation Architecture](architecture/IMPLEMENTATION_ARCHITECTURE.md)
- [Target Domain Model](architecture/DOMAIN_MODEL.md)
- [Retention and Deletion](architecture/RETENTION_AND_DELETION.md)
- [Provider and Authoritative Source Registry](integrations/SOURCE_REGISTRY.md)

### Security

- [Security design documentation](security/README.md)
- [Phase 0 Threat Model](security/PHASE_0_THREAT_MODEL.md)

### Product

- [Product documentation](product/README.md)
- [Product Scope](product/PRODUCT_SCOPE.md)

### Product design

- [Design documentation](design/README.md)
- [Brand identity and design system](design/DESIGN_SYSTEM.md)

### Integrations

- [Integration documentation](integrations/README.md)
- [Provider and Authoritative Source Registry](integrations/SOURCE_REGISTRY.md)

### Development

- [Development documentation](development/README.md)
- [Capability-to-client support matrix](development/CAPABILITY_TO_CLIENT_SUPPORT.md)
- [Implementation Roadmap](development/IMPLEMENTATION_ROADMAP.md)
- [Delivery traceability evidence index](development/DELIVERY_TRACEABILITY.md)

## Repository policies

These intentionally remain at the repository root:

- [`CONTRIBUTING.md`](../CONTRIBUTING.md)
- [`SECURITY.md`](../SECURITY.md)
- [`BRANDING.md`](../BRANDING.md)
- [`TRADEMARKS.md`](../TRADEMARKS.md)
- [`LICENSE`](../LICENSE)
- [`NOTICE`](../NOTICE)

Future API, MCP, self-hosting, operations and user documentation should be added in clearly named subdirectories here rather than at the repository root.
