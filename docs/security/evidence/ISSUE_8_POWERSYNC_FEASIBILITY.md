...
**Scope:** commit-bound authenticated scope/revocation, experimental reconciliation, retention, honest-client reset, bounded-capacity, cross-process restart, cached/offline-read and adversarial checkpoint-characterization observations; native and owner/security decision gates remain separate
...
The technical conclusion is **do not use checkpoint acknowledgement as authority**. Server controls prevent several stale or unauthorised outcomes, but they do not enforce the canonical hostile-client `P90D` boundary for unchanged authorised state, prove local wipe, or prevent unseen intent rebinding. The separate [checkpoint-attestation decision packet](ISSUE_8_POWERSYNC_CHECKPOINT_RISK.md) records the 2026-08-11 owner direction to preserve the policy and pursue a backend-verifiable alternative. The subsequent [platform assessment](ISSUE_8_BACKEND_VERIFIABLE_CHECKPOINT_ALTERNATIVE.md) found no qualifying public general Android/macOS storage-owning attestation primitive. Neither document records security, legal or production acceptance.
...
- R5a proves the checkpoint/reset acknowledgement gap with a raw HTTP-only replica: a false checkpoint restores incremental eligibility and a reset ack cannot prove clear. Current authorisation/version/incarnation/idempotency checks remain separate, but the canonical trusted-checkpoint gate is not met. The R5b backend-verifiable direction was assessed against PowerSync and a dated inventory of public Android/macOS trust primitives; no qualifying general-client implementation path was identified and owner follow-up remains pending.
...
| `TH-SYNC-004` / `MIT-SYNC-004` | strict endpoint-time retention, honest-client reset/full sync, cross-process pre-connect recovery, revocation/incarnation-filtered quarantine and adversarial proof that client ack/reset is not attestation | `executed-uncommitted` | no qualifying general-client storage-bound attestation primitive identified; owner follow-up, permanently offline/hostile-device acceptance, node/volume loss and encrypted/forensic/key deletion remain `not-validated` |
...
3. Resolve the bounded negative R5b [platform-capability assessment](ISSUE_8_BACKEND_VERIFIABLE_CHECKPOINT_ALTERNATIVE.md): restrict support to a future qualified storage-owning trusted environment and validate it, reject connected-sync support for the intended general clients with product/support migration, offline-mode consequences and replacement user-flow acceptance evidence, or explicitly revise the canonical policy with named residual-risk acceptance. Production sizing/distributed enforcement and quarantine encryption remain gates under Issues #45/#9; standalone local-only authority remains separately gated by Issues #2/#9.
