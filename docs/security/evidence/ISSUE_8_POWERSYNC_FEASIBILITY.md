...
**Evidence status:** commit-bound M2, M3a and M3b-R1/R2/R3/R4/R5a executions succeeded locally; complete Issue #8 validation is not claimed

**Scope:** commit-bound authenticated scope/revocation, experimental reconciliation, retention, honest-client reset, bounded-capacity, cross-process restart, cached/offline-read and adversarial checkpoint-characterization observations; native, legal, production and independent security-review gates remain separate
...
### 3.6 M3b-R5a commit-bound adversarial checkpoint characterization

The following later run exercised clean candidate commit `b314ead79cb530ac8ebabf5d31e0096b7b7e552f`. Its ignored local record remains `executed-uncommitted`; the verdict is `limitation-demonstrated`, not attestation or risk acceptance:

```text
run ID: 1fb0f8db-3938-413e-bb3a-8e97cdb5f68b
Compose project: trax-ps8-maurice-1fb0f8db-3938-413e-bb3a-8e97cdb5f68b
Linux x86_64; Node.js v22.23.2; pinned npm 10.9.4
Docker client/server 29.7.2; Docker Compose 5.4.0
wrapper exit: 0; unit tests: 24/24; integration subtests: 2/2; R4 phases and cleanup: succeeded
source digest: 26e320a138699743e2d81f82e1a0c8188a6e72f571a62bb8cb488c9ffaa582b6
```

A raw registered replica used authenticated HTTP only and never opened PowerSync or SQLite. The sanitized negative-capability observations recorded:

- an upload at `P90D` plus one microsecond first received HTTP 428 `replica_reset_required`;
- immediate issue/ack of a client-observed checkpoint challenge without sync changed the reset predicate to false, after which an unchanged, still-authorised old update applied;
- current authorisation, expected version, immutable incarnation, missing-target denial and prior server-observed command digest/idempotency each remained independently authoritative with exact full-resource and receipt/event effects;
- raw reset acknowledgement rotated the epoch without local clear/full sync and retired the old credential, but a never-server-observed pre-reset intent could be re-enveloped under epoch 2 and apply when current checks matched;
- the missing and existing-but-revoked denial bodies had exact equal structure after only independently verified request-derived command/resource/digest fields were normalised;
- no create command exists in the spike protocol, so create identity, uniqueness and resurrection behavior were not tested;
- retained R5 evidence contains exact codes/counts/booleans only and preserves its commit-bound execution-time values `riskAcceptance: pending-human-decision` and `blocked-pending-alternative-or-policy-revision`; the later owner direction does not rewrite historical evidence.

The technical conclusion is **do not use checkpoint acknowledgement as local-state authority**. Server controls prevent several stale or unauthorised outcomes, but they do not prove hostile-client `P90D` lifecycle compliance, local wipe or absence of unseen intent rebinding. The separate [checkpoint-attestation decision packet](ISSUE_8_POWERSYNC_CHECKPOINT_RISK.md) now records the owner's later [ADR-018](../../architecture/decisions/ADR-018-CONNECTED-SYNC-TRUST-BOUNDARY.md) policy revision after the [platform assessment](ISSUE_8_BACKEND_VERIFIABLE_CHECKPOINT_ALTERNATIVE.md) found no qualifying public general Android/macOS storage-owning attestation primitive. That owner product-risk choice does not rewrite the historical run values or constitute independent security, legal or production acceptance.
...
- R5a proves the checkpoint/reset acknowledgement gap with a raw HTTP-only replica: a false checkpoint restores incremental eligibility and a reset ack cannot prove clear. Current authorisation/version/incarnation/idempotency checks remain separate. The R5b backend-verifiable direction was assessed against PowerSync and a dated inventory of public Android/macOS trust primitives; no qualifying general-client implementation path was identified. The owner later accepted ADR-018's server-eligibility/honest-client lifecycle boundary. A production stale epoch must not regain normal eligibility without reset/rotation/full-sync lifecycle, even though a hostile client can still report that lifecycle falsely.
...
| `TH-SYNC-004` / `MIT-SYNC-004` | strict endpoint-time retention, honest-client reset/full sync, cross-process pre-connect recovery, revocation/incarnation-filtered quarantine and adversarial proof that client ack/reset is not attestation | `executed-uncommitted` | ADR-018 owner product-risk decision recorded; production eligibility state machine, independent hostile-device/security acceptance, node/volume loss and encrypted/forensic/key deletion remain `not-validated` |
...
3. Implement and independently review the owner-approved [ADR-018](../../architecture/decisions/ADR-018-CONNECTED-SYNC-TRUST-BOUNDARY.md) policy revision: endpoint-owned eligibility/epoch state, exact `P90D`/floor transitions, stale-epoch reset/rotation/full-sync lifecycle, independent canonical command checks, create identity allocation and explicit no-remote-wipe behavior. Production sizing/distributed enforcement and quarantine encryption remain gates under Issues #45/#9; standalone local-only authority remains separately gated by Issues #2/#9.
