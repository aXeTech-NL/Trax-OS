...
The technical conclusion is **do not use checkpoint acknowledgement as authority**. Server controls prevent several stale or unauthorised outcomes, but they do not enforce the canonical hostile-client `P90D` boundary for unchanged authorised state, prove local wipe, or prevent unseen intent rebinding. The separate [checkpoint-attestation decision packet](ISSUE_8_POWERSYNC_CHECKPOINT_RISK.md) records the 2026-08-11 owner direction to preserve the policy and pursue a backend-verifiable alternative. It records no concrete-alternative, security, legal or production acceptance.
...
- R5a proves the checkpoint/reset acknowledgement gap with a raw HTTP-only replica: a false checkpoint restores incremental eligibility and a reset ack cannot prove clear. Current authorisation/version/incarnation/idempotency checks remain separate, but the canonical trusted-checkpoint gate is not met. The R5b owner direction now requires a backend-verifiable alternative; concrete architecture, independent security validation and production evidence remain pending.
...
| `TH-SYNC-004` / `MIT-SYNC-004` | strict endpoint-time retention, honest-client reset/full sync, cross-process pre-connect recovery, revocation/incarnation-filtered quarantine and adversarial proof that client ack/reset is not attestation | `executed-uncommitted` | selected backend-verifiable alternative design/validation, permanently offline/hostile-device acceptance, node/volume loss and encrypted/forensic/key deletion remain `not-validated` |
...
3. Complete R5b using the owner-selected backend-verifiable direction in the [checkpoint-attestation decision packet](ISSUE_8_POWERSYNC_CHECKPOINT_RISK.md): specify the concrete alternative, validate durable applied-watermark binding plus replay/copied-client/failure recovery, obtain independent security acceptance and attach immutable production evidence. Production sizing/distributed enforcement and quarantine encryption remain gates under Issues #45/#9; standalone local-only authority remains separately gated by Issues #2/#9.
