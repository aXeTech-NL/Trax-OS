...
**Model:** `TM-PHASE0-001` version `1.0.0`
...
- **Non-goals:** implementing those future systems, choosing cryptographic suites/token formats/offline leases/topology, reviving browser-local authority, or treating planned evidence as proof.
...
| `TB-SYNC-004` Revocation and tombstones to device purge | `not-implemented` | Authoritative revocation state → Potentially offline cached data | `TH-SYNC-004` A permanently offline or hostile device retains previously authorized data and keys after revocation. | `MIT-SYNC-004` Invalidate online grants immediately, retain tombstones for the supported offline window and purge/recheck on reconnect; disclose endpoint residual risk. | `EV-SYNC-PLANNED` (planned) | `RR-SYNC-004` medium; pending explicit Phase 0 acceptance |
