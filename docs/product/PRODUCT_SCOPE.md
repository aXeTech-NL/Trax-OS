...
Connected sync through Trax Cloud or a compatible self-hosted endpoint supports incremental offline reconciliation through exactly 90 consecutive days after the last server-recorded eligibility checkpoint. The endpoint owns the time, replica epoch, retention target and eligibility decision; the official client reports locally observed completion. Incremental local-first sync is not supported beyond that interval or below the retained graveyard floor. A stale connected replica must reconnect, preserve still-authorised pending commands for review/export, rotate/reset its sync replica and complete a full resync before connected sync resumes. Pending changes are never silently uploaded or discarded during that reset; commands for a revoked scope are explicitly denied and securely purged instead.

The lifecycle report is not proof that a hostile or copied endpoint durably applied or cleared data. Server revocation stops future access but Trax OS does not claim remote wipe of previously disclosed local copies. Current authorisation, replica epoch, version, incarnation and idempotency are checked independently for every upload under [ADR-018](../architecture/decisions/ADR-018-CONNECTED-SYNC-TRUST-BOUNDARY.md).
